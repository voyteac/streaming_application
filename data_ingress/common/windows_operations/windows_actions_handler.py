...
class WindowsActionsHandler:
    def __init__(self):
        self.data_generation_pause_period: float = data_generation_pause_period
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self.check_port_status(result.stdout, self.server_port):
                    log_debug(self.check_TCP_port_data_generation_with_retries, f'Data is generated on port: {self.server_port}!')
                    return True
                else:
