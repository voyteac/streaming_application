...
from data_ingress.database_handling.database_exception import KafkaMessageToDatabaseMessageConversionFailed
from data_ingress.models import MetricsDataModelsLoader


def compose_db_message_based_on_kafka_msg(kafka_msg: ConsumerRecord) -> MetricsDataModelsLoader:
    timestamp_generator: RealTimestampGenerator = RealTimestampGenerator()

    log_debug(compose_db_message_based_on_kafka_msg, f'Kafka message to database_handling message conversion ...')
    try:
        ...
        log_error_traceback(compose_db_message_based_on_kafka_msg)
        raise KafkaMessageToDatabaseMessageConversionFailed(str(e), compose_db_message_based_on_kafka_msg,
                                                            kafka_msg) from e
    else:
        log_debug(compose_db_message_based_on_kafka_msg,
