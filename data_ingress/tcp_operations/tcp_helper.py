...
def create_tcp_socket() -> socket.socket:
    ...
    log_debug(create_tcp_socket, 'TCP socket created')
    return tcp_socket
...
def connect_to_tcp_socket(tcp_socket: socket.socket, host: str, port: int) -> socket.socket:
    try:
        ...
        log_debug(connect_to_tcp_socket, f'Connected to {host}:{port}')
        return tcp_socket
    except socket.error as e:
        log_error(connect_to_tcp_socket, f'Error connecting to {host}:{port} - {e}')


def close_tcp_socket(tcp_socket: socket.socket) -> None:
    tcp_socket.close()
    log_debug(close_tcp_socket, 'TCP connection closed')
...
def receive_data_via_tcp(tcp_connection: socket.socket, buffer_size: int) -> bytes:
    ...
    log_debug(receive_data_via_tcp, f'received_data: {received_data}')
...
def send_message_to_tcp_socket(tcp_socket: socket.socket, message: bytes) -> None:
    try:
        ...
        log_debug(send_message_to_tcp_socket, 'Message sent!')
    except socket.error as e:
        log_error(send_message_to_tcp_socket, f'Error sending message: {e}')


def accept_tcp_connection(tcp_socket: socket.socket) -> socket.socket:
    tcp_connection, tcp_address = tcp_socket.accept()
    log_debug(accept_tcp_connection, f'TCP socket opened with address: {tcp_address}')
